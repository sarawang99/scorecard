def ChiMerge_con(df, var, flag, confidenceVal=3.841, maxbin=10,minBinPcnt=0):
    '''变量分箱
    Parameters
    -----------
    ...
    while (1):
        if (len(chi_table)!=0):
            ...
            np_regroup = np.delete(np_regroup, chi_min_index + 1, 0) #删除指定行

            '''
            计算合并后分组相关联的卡方值并替换
            1、当chi_min_index位于chi_table最后一个时：计算合并后分组与前一个分组的卡方值
            2、当chi_min_index位于chi_table第一个时：计算合并后分组与后一个分组的卡方值
    ...
    for i in np.arange(np_regroup.shape[0]):
        if i == 0:
            x = '(-inf' + ',' + str(np_regroup[i, 0])+']'
        elif i == np_regroup.shape[0] - 1:
            x = '('+str(np_regroup[i - 1, 0]) + ',inf)'
        else:
    ...
    result_data['var'] = [var] * len(cutpoint)  # 结果表第一列：变量名
